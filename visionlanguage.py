...
class OCRConflictGenerator(LLMObject):
    ...
    def generate_from_text(self,text,name_prefex:str="",font:str = "arial.ttf",sleep:float=0.1):
        c = 0
        for act in self.LLM_ACTION:
            for i in act:
                ...
                name = name_prefex+f"-{c}.png"
                self.generate_png(inst=i,text=text,name=name,text_color=color1,background_color=color2,font=font,fontsize=fontsize,delta1=delta1,delta2=delta2)
                for j in act:
                    if i == j:
                        continue   
                    current_dict = {"sentence":text,"inst1":i,"inst2":j,"path":name}
                    with open("Ocrconflict.jsonl","a") as f:
    ...
    def create(self,sleep:float=0.1):
        ...
        for s in tqdm.tqdm(self.sentence):
            if not os.path.exists('ocrconflict'):
                os.makedirs('ocrconflict')
            self.generate_from_text(text=s,name_prefex=f"ocrconflict\{out_c}")
            time.sleep(sleep)
            out_c = out_c + 1
...
class FigureConflictGenerator(LLMObject):
    ...
    def _draw_graph_for_single_dict(self,path:str,sdict:dict,ndict:dict,dpi:int=200):
        if sdict["type"] == "linechart":
            ...
            #print(f"Origin:{font}")
            path = "figureconflict/Manipulated-" + path.lstrip("figureconflict/")
            self._drawing_line(path=f"{path}",data=ndict["data"],x=x,y=y,title=sdict["title"],color=color,figsize=figsize,font=font,lfs=lfs,tfs=tfs,border=border,dpi=dpi)
            ...
            print(f"Origin:{font}")
            path = "figureconflict/Manipulated-" + path.lstrip("figureconflict/")
            self._drawing_pie(path=f"{path}",data=ndict["data"],title=sdict["title"],colors=colors,figsize=figsize,font=font,lfs=lfs,tfs=tfs,border=border,dpi=dpi)
            print(f"Manipulated:{font}")
            ...
            print(f"Origin:{font}")
            path = "figureconflict/Manipulated-" + path.lstrip("figureconflict/")
            self._drawing_bar(path=f"{path}",data=ndict["data"],title=sdict["title"],color=color,figsize=figsize,font=font,lfs=lfs,tfs=tfs,border=border,dpi=dpi)
            print(f"Manipulated:{font}")
    
    def create(self,sleep:float=0.1,dpi:int=200):
        if not os.path.exists('figureconflict'):
            os.makedirs('figureconflict')
        for i in tqdm.trange(len(self.established_dict)):
            ...
            try:
                ...
                success = 0
            try:
                self._draw_graph_for_single_dict(sdict=sdict,ndict=ndict,dpi=dpi,path=f"figureconflict/{clean_title}-{clean_entity}{path}")
            except:
                success = 0
            q = self._question_relate(entity_dict=sdict)
            d = self._corresponding_description(entity_dict=sdict)
            p1 = f"figureconflict/{clean_title}-{clean_entity}{path}"
            p2 = f"figureconflict/Manipulated-{clean_title}-{clean_entity}{path}"
            stored_dict = {"question":q,"language":d,"original_figure":p1,"manipulated_figure":p2,"success_manipulate":success}
            with open("Figureconflict.jsonl","a") as f:
...
class GeometricConflictGenerator():
    ...
    def create(self,num:int=2000):
        data_list=[]
        if not os.path.exists('geometricconflict'):
            os.makedirs('geometricconflict')
        ...
        for no in range(num):
            ...
            plt.savefig('geometricconflict/image' + str(no) + '.png')
            plt.close()
            
            data['questions'] = self._questionsGenerator(data)
            data_list.append(data)

        with open('geometricconflict/dataInfo' + '.json', 'a') as f:
            json.dump(data_list, f)
        

    def choose(self,path:str="geometricconflict/dataInfo.json"):
        with open(path,"r") as f:
            ...
            for data in full_data:
                ...
                with open("Geometricconflict.jsonl","a") as f2:
                    json.dump(data,f2)
                    f2.write('\n')
...
class SemanticConflictGenerator(LLMObject):
    ...
    def create(self):
        ...
        for i in range(8, len(IMAGENET2012_CLASSES)):
            ...
            dataset.append(item)
        with open('dataset_classification.json', 'a') as f:
            json.dump(dataset, f)

    def choose(self,path:str="dataset_classification.json"):
        ...
        with open(path,"r") as f:
            ...
            for i,data in enumerate(d):
                ...
                for i in list_i:
                    ...
                    with open("Semanticconflict.jsonl","a") as f2:
